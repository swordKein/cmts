...
public class CcubeService implements CcubeServiceImpl {
    ...
    private JsonObject getTagsMetasObjWithResultTag(JsonObject origObj, List<ItemsTags> tagsMetasList) throws Exception {
        if(origObj == null) origObj = new JsonObject();
        ...
        for(String type : origTypes) {
            ...
            String otype = type.replace("METAS","");
            ...
            if (tagsMetasMap != null && tagsMetasMap.get(type) != null) {
                ...
                if (tmpList != null && tmpList.size() > 0) {
                    ...
                    for (String ts : tmpList) {
                        List<String> tsWithResultTags = dicService.getStringArrayFromWordWithResultTag(ts, otype);

                        if (tsWithResultTags != null && tsWithResultTags.size() > 0) {
                            for (String tswt : tsWithResultTags) {
                                tmpList2.add(tswt);
                            }
                        } else {
                            tmpList2.add(ts);
                ...
                // 중복 제거 추가 18.05.03
                List<String> tmpList3 = StringUtil.removeDupListString(tmpList2);

                if (tmpList3 != null) System.out.println("#MLOG.tmpLIST3:"+tmpList3.toString());

                // List를 구분자로 구분하여 String으로 치환, 현재 delimeter는 comma
                String meta = StringUtil.convertArrayStringToStringAddDelimeter(tmpList3, ",");
    ...
    @Override
    public JsonArray getJsonArrayForCcubeOutputWithResultTag(JsonArray contentsArr, String type, Map<String, Object> reqMap) throws Exception {
        if (contentsArr == null) contentsArr = new JsonArray();
        ...
        if (reqMap != null) {
            ...
            if (itemIdx > 0) {
                ...
                if (itemInfo != null) {
                    ...
                    // items_tags_metas를 읽어와서 Obj에 매핑
                    newItem = this.getTagsMetasObjWithResultTag(newItem, itemInfo.getTagsMetasList());

                    //newItem = this.getMetaTagsWithResultTag(newItem);
