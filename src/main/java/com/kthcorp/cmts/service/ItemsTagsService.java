...
public class ItemsTagsService implements ItemsTagsServiceImpl {
    ...
    public Integer processMetaObjectByTypes(JsonObject origMetasArraysByType, JsonObject actionItemsArraysByType, JsonArray typesArr
        ...
        int rt = 0;
        System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 시작");
        ...
        System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타사전 시작");
        ...
        System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타사전 끝");
        
        System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 시작");
        if (typesArr != null && origMetasArraysByType != null && actionItemsArraysByType != null) {
            ...
            System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes setEmptyMetas 시작");
            JsonObject origMetasArraysByType2 = setEmptyMetas(origMetasArraysByType, this.getOrigTypes());
            System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes setEmptyMetas 끝");
            
            System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 시작");
            for (String atype : this.getOrigTypes()) {
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 : atype = " + atype + " 시작");
                ...
                if (changeMetaArr != null) {
                	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 : atype = " + atype + " - changeMetaArr 널 아님");
                    ...
                    if (destArr != null) {
                        ...
                        if (!atype.toUpperCase().contains("AWARD")) {
                            ...
                            System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 : atype = " + atype + " insItemsTagsMetas 동작 전 : mtype,meta = " + atype+","+destMeta);
                            System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 : atype = " + atype + " insItemsTagsMetas 동작 전 : reqMeta = " + reqMeta);
                            rt = this.insItemsTagsMetas(reqMeta);//저장시 메타키워드 누락 테스트 주석달음!!!!
                            System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 : atype = " + atype + " insItemsTagsMetas 동작 후 : rt = " + rt);
                        } else {
                            ...
                            System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 : atype = " + atype + " insItemsTagsMetas 동작 전 : mtype,meta = " + atype+","+destMeta);
                            System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 : atype = " + atype + " insItemsTagsMetas 동작 전 : reqM = " + reqM);
                	...
                	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 : atype = " + atype + " - changeMetaArr 널 맞음");
                    if (origMetaArr != null) {
                ...
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 : atype = " + atype + " 끝");
            }
            System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 끝");
        }
        System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 메타 반복문 끝");
        System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.processMetaObjectByTypes 리턴 : rt = " + rt);
        return rt;
    }
    ...
    public int changeMetasArraysByTypeFromInputItems (int itemid, String items, String duration, String sendnow) {
		System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems 시작");
		System.out.println(" - itemid = " + itemid);
        ...
        try {
            ...
            System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - input asfasfd = " + items);
            JsonObject actionItemsArraysByType = this.getArraysByTypeFromInputItems(items);
            System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - output asfasfd = " + actionItemsArraysByType.toString());
            System.out.println("#actionItemsArraysByType:"+actionItemsArraysByType.toString());
            ...
            if(lastTag != null && !"S".equals(lastTag.getStat())) {
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 기승인된 메타가 없을 경우");
            	
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - getItemsMetasByItemIdx 실행 전");
            	JsonObject origMetasArraysByType = this.getItemsMetasByItemIdx(itemid, false);
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - getItemsMetasByItemIdx 동작 후 : origMetasArraysByType = " + origMetasArraysByType.toString());

                // AWARD 처리를 위해 ITEMS_METAS에서 읽어서 구조체에 추가
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - getAwardObject 동작 전(호출) : AWARD 처리를 위해 ITEMS_METAS에서 읽어서 구조체에 추가");
                origMetasArraysByType = this.getAwardObject(itemid, origMetasArraysByType);
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - getAwardObject 동작 후 : origMetasArraysByType = " + origMetasArraysByType.toString());

                System.out.println("#origMetasArraysByType:"+origMetasArraysByType.toString());

                /* action_item이 있는 경우 타입별 meta 수정 */
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - processMetaObjectByTypes 동작 전(호출) action_item이 있는 경우 타입별 meta 수정");
                int rtm = this.processMetaObjectByTypes(origMetasArraysByType, actionItemsArraysByType, typesArr, itemid, curTagIdx);
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - processMetaObjectByTypes 동작 후 : rtm = " + rtm);
            	System.out.println(" - origMetasArraysByType " + origMetasArraysByType.toString());
            	System.out.println(" - actionItemsArraysByType " + actionItemsArraysByType.toString());
            	...
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - uptItemsTagsKeysStat 동작 전");
                int rts = this.uptItemsTagsKeysStat(reqConfirm);
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - uptItemsTagsKeysStat 동작 후 : rts = " + rts);

                /* 해당 items_stat 를 승인으로 업데이트 한다 */
                reqIt = new Items();
                reqIt.setIdx(itemid);
                reqIt.setStat("ST");
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - itemsMapper.insItemsStat 동작 전");
                int rti = itemsMapper.insItemsStat(reqIt);
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - itemsMapper.insItemsStat 동작 후 : rti = " + rti);

                rt = 1;

                tagcnt = curTagIdx + 1;
            } else {
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 이전 상태 코드 확인하여 승인완료(ST)가 아닌 경우 승인완료로 처리  added 2018.04.11");
                /* 이전 상태 코드 확인하여 승인완료(ST)가 아닌 경우 승인완료로 처리  added 2018.04.11 */
                String oldItemsStat = itemsMapper.getItemsStatByIdx(itemid);
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - oldItemsStat " + oldItemsStat);
                if (!"ST".equals(oldItemsStat)) {
                	...
                	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - curTagIdx " + curTagIdx);
                ...
                /* action_item이 있는 경우 타입별 meta 수정 */
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - processMetaObjectByTypes 동작 전");
                int rtm = this.processMetaObjectByTypes(origMetasArraysByType, actionItemsArraysByType, typesArr, itemid, curTagIdx);
            	System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - processMetaObjectByTypes 동작 후 : rtm = " + rtm);
            	System.out.println(" - origMetasArraysByType " + origMetasArraysByType.toString());
            	System.out.println(" - actionItemsArraysByType " + actionItemsArraysByType.toString());
            ...
            System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 직전");
            if (rt > 0) {
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0");
                /* 해당 items_tags_keys 를 승인으로 업데이트 한다  */
                ItemsTags reqConfirm = new ItemsTags();
                reqConfirm.setIdx(itemid);
                reqConfirm.setTagidx(curTagIdx);
                reqConfirm.setStat("S");
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - uptItemsTagsKeysStat 실행 전");
                System.out.println(" - itemid = " + itemid);
                System.out.println(" - curTagIdx = " + curTagIdx);
                System.out.println(" - stat = S");
                int rts = this.uptItemsTagsKeysStat(reqConfirm);
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - uptItemsTagsKeysStat 실행 후 : rts = " + rts);
                System.out.println(" - itemid = " + itemid);
                System.out.println(" - curTagIdx = " + curTagIdx);
                System.out.println(" - stat = S");
                ...
                reqIt = new Items();
                reqIt.setIdx(itemid);
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - getItemsByIdx 실행 전");
                Items itemInfo = itemsService.getItemsByIdx(reqIt);
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - getItemsByIdx 실행 후 : itemInfo = " + itemInfo.toString());
                String movietitle = "";
                movietitle = (itemInfo != null && itemInfo.getTitle() != null) ? itemInfo.getTitle().trim() : "";
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - insItemsHist 실행 전");
                int rthist = itemsService.insItemsHist(itemid, "meta", "S", movietitle, "CONFIRM_META", itemid);
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - insItemsHist 실행 후 : rthist = " + rthist);
                ...
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - 해당 items 정보를 변경한다");
                if (!"".equals(duration)) reqIt.setDuration(duration);
                reqIt.setTagcnt(tagcnt);
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - uptItemsTagcnt 실행 전");
                int rtu = itemsService.uptItemsTagcnt(reqIt);
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - uptItemsTagcnt 실행 후 : rtu = " + rtu);
                logger.info("#MLOG:uptItemsTagcnt for itemIdx:"+itemid);

                /* 해당 items의 sched_target_content 원문을 모두 삭제한다  18.05.16 */
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - deleteSchedTargetContentOrigin 실행 전");
                int rtd = schedTriggerService.deleteSchedTargetContentOrigin(itemid);
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - deleteSchedTargetContentOrigin 실행 후 : rtd = " + rtd);
            }

            if ("Y".equals(sendnow.toUpperCase())) {
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 sendnow=Y");
                Map<String,Object> reqCcube = new HashMap();
                reqCcube.put("idx", itemid);
                reqCcube.put("regid", serverid);
                System.out.println(" - itemid = " + itemid);
                System.out.println(" - serverid = " + serverid);
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - insCcubeOutput 실행 전");
                int rtc = ccubeService.insCcubeOutput(reqCcube);
                System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems - 저장액션 rt > 0 - insCcubeOutput 실행 후 : rtc = " + rtc);
            }
		...
		System.out.println("[저장시 메타키워드 누락 테스트] ItemsTagsService.changeMetasArraysByTypeFromInputItems 끝 : rt = " + rt);
        return rt;
    }
